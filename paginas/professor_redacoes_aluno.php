...
<html lang="en">
  <head>
	<title>Minha Redação - Todas as Redações</title>
    <meta charset="utf-8">
...
<div class="app-title">
    ...
    $busca_sem_corrigir = mysqli_query($conn,"SELECT * FROM redacao where codigo_tema='$codigo_tema' and nota = 0 or nota is null") ;
    ...
    $pesq = mysqli_query($conn,"SELECT * FROM temas_redacao where id='$codigo_tema' and codigo_professor = '".$row['id']."'");
    $nome_tema = mysqli_fetch_assoc($pesq);
    if(mysqli_num_rows($pesq) >= 1){
			?>
          <div class="tile">
         <center> <h3 class="tile-title">Todas as redações do tema: <?php echo $nome_tema['nome_tema']?></h3>
         <a href="../dados/dados_redacao.php?codigo_tema=<?php echo $redacao['codigo_tema']?>"><button class="btn btn-info" style="background:orange;border-color:orange;" type="button">Estatisticas</button></a>
           ...
           <th>Deletar Redação</th>
                ...
                <tbody>
				   <?php
					...
					?>
						<h2 align="center" style="color:red;">Esse tema ainda não possui redações</h2>
					<?php
						}else{
              ...
              $turma = mysqli_fetch_assoc($busca_turma);
					?> 
                  <tr>
                    <td><img src="<?php echo $row['foto_aluno']?>" style="border-radius:100%;width:50px;height:50px;cursor:pointer;" data-toggle="modal" data-target="#modal_aluno_<?php echo $row['codigo_aluno']?>"></td>
					...
					<?php }?></td>
                    <td><a href="professor_ver_redacao.php?codigo_redacao=<?php echo $row['id']?>"><button class="btn btn-success" type="button">Abrir <img src="../img/icone/menu/folha.png" width="16px" height="16px"></button></a></td>
					...
					?>
				    <td><a href="professor_ver_correcao.php?codigo_correcao=<?php echo $row['id']?>" title="Parece que essa redação ainda não tem correção"><button class="btn btn-info" type="button" disabled  >Ver <img src="../img/icone/menu/olho.png" width="16px" height="16px"></button></a></td>
					<?php
					}else{
					?>
					<td><a href="professor_ver_correcao.php?codigo_correcao=<?php echo $row['id']?>"><button class="btn btn-info" type="button">Ver <img src="../img/icone/menu/olho.png" width="16px" height="16px"></button></a></td>
					<?php
					}
					?>				
        
        	<td><?php echo date("d/m/Y \à\s H:i:s", strtotime($row['data_envio'])); ?></td>
				 <td><button class="btn" style="color:#FFF;background-color:red;" data-toggle="modal" data-target="#<?php echo $row['id'];?>" >Deletar Redação <img src="../img/icone/lixo.png" width="16px" height="16px"></button></td>
            ...
            <!--Janela Modal que exclui o aluno-->
                  ...
                  <h5>Tem certeza que deseja excluir está redação?</h5>
								</div>
								<div class="modal-body" align="center">
								<a href="../php/professor_exclui_redacao.php?codigo_redacao=<?php echo $row['id'];?>&codigo_tema=<?php echo $row['codigo_tema'];?>" ><button class="button-modal" width="110px" style="background:green;border-color:green;"  type="button">SIM</button></a>
                ...
                <a href=""><button class="button-modal" data-dismiss="modal" style="background:red;border-color:red;"  type="button">NÃO</button></a>
                  ...
                  <!--Janela Modal que mostra o aluno-->
								<div class="modal fade" id="modal_aluno_<?php echo $row['codigo_aluno'];?>" role="dialog">
								<div class="modal-dialog">
								<div class="modal-content">
                  <br>
								<div class="modal-body" align="center">
    ...
    <?php
			...
			<h1 style="color:red;font-weight:bold;">Você não tem acesso a essas redações</h1>
