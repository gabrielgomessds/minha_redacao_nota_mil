...
<html lang="en">
  <head>
	<title>Minha Redação - Rank Alunos</title>
    <meta charset="utf-8">
...
<div class="app-title">
        <div>
          <h1><a href="aluno.php"><button class="btn" style="background-color:green;border-color:green;color:#FFF;">Voltar</button></a> Rank <img src="../img/icone/medalha_rank.png" width="40px" height="40px"></h1>     
    ...
    $buscaTema = mysqli_query($conn,"SELECT * FROM temas_redacao where id='$codigo_tema' ") ;
		$tema = mysqli_fetch_assoc($buscaTema);
          ...
          $rank = mysqli_fetch_assoc($buscarank);
					...
					?> 
          <?php
            if($rank['codigo_turma'] == 'todasturmas'){
...
?>
  <!--####################################################################################################################################################################################-->
            <?php
          }else if($rank['codigo_turma'] == 'turma'){
            $buscaRedacoes = mysqli_query($conn,"select * from redacao where codigo_tema = '".$rank['codigo_tema']."' and codigo_turma = '".$aluno['codigo_turma']."'");
    ...
    <!--####################################################################################################################################################################################-->
          ...
          <?php
              ...
              $buscaRedacoes = mysqli_query($conn,"select * from redacao where codigo_tema = '".$rank['codigo_tema']."' and codigo_turma = '".$aluno['codigo_turma']."'");
              $buscaTurma = mysqli_query($conn,"select * from turma where id = '".$aluno['codigo_turma']."'");
              $turma = mysqli_fetch_assoc($buscaTurma);
          ?>
    <center><h4><b style="color:blue;">Classificação da turma do <?php echo $turma['nome_turma'];?><br></b></h4></center><br>
<a href="rank_geral.php?codigo_tema=<?php echo $rank['codigo_tema']?>"><button class="btn" style="background-color:#EE7600;color:#FFF;border-color:#EE7600;">Ver na classificação geral</button></a><br><br><br>
          <?php
           while( $pegarank = mysqli_fetch_assoc($buscaRedacoes)){
            ?>
             <tr>
